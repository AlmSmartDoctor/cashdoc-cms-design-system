...
## 4. JSDoc 표준 (IDE 내 문서화)

컴포넌트 JSDoc에는 아래 항목을 포함합니다.

1. 컴포넌트 개요(역할/핵심 가치)
2. `When`: 사용해야 하는 경우 / 사용하지 말아야 하는 경우
3. `Layout behavior`: 레이아웃/동작 특성
4. `Usage guidelines`: Do / Don't
5. `Accessibility`: 키보드/ARIA 정보
6. `Example`: `{@tool snippet}` 기반 대표 예시
7. 참고 사진 링크
...
3. 스크린샷 캡처 후 지정 저장소 경로에 업로드
4. JSDoc 표준 섹션 및 스크린샷 링크 작성
5. `pnpm lint`, `pnpm type-check` 통과 확인
6. 영향도에 맞는 버전 의도로 커밋/PR 작성
...
VS Code `User Snippets`에 등록해 사용하세요.
